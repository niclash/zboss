...
wait_for_start() {
    ...
    do
        ...
        s=`grep Device aps_dups_rej_${nm}*.log`
    done
    if echo $s | grep OK
    then
        return
...
./aps_dups_rej_zc /tmp/aaa0.write /tmp/aaa0.read &
coordPID=$!
wait_for_start zc
...
echo "run ze"
./aps_dups_rej_ze /tmp/aaa1.write /tmp/aaa1.read &
endPID=$!
wait_for_start ze
...
if grep "is a dup - drop" aps_dups_rej_ze*.log
then
  echo "SUCCESS"
else
  echo "FAILED"
